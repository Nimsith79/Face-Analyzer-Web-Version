...
export function generateFacialResults(faceLandmarks, faceBlendshapes) {
  ...
  if (!landmarks || !faceBlendshapes?.length || !g) {
    return {
      ...
      limitations: [
        "No stable face landmarks or blendshape scores were detected.",
        "Some features like true bone structure cannot be directly measured from this model alone."
      ]
    };
  }
  ...
  return {
    ...
    limitations: [
      "These are heuristic estimates from 2D landmarks and blendshape scores, not medical measurements.",
      "Cheekbones and jawline bone structure are approximated from soft-tissue geometry and expressions.",
      "Lighting, camera angle, head rotation, and facial expression can strongly affect the output."
    ]
  };
}
