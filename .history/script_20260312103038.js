...
import { generateFacialResults } from "./logic.js";
...
async function handleClick(event) {
  ...
  renderGeneratedResult("image", result.faceLandmarks, result.faceBlendshapes);
...
async function predictWebcam() {
  ...
  if (results?.faceBlendshapes?.length && now - lastVideoAnalysisMs > 250) {
    ...
    renderGeneratedResult("video", results.faceLandmarks, results.faceBlendshapes);
  }
...
function renderGeneratedResult(mode, faceLandmarks, faceBlendshapes) {
  const data = generateFacialResults(faceLandmarks, faceBlendshapes);
