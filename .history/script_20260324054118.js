...
let lastServerAnalysisMs = 0;

// ====== OPTIONAL SERVER SCORING ======
// If set, the client will POST frames + landmarks to this endpoint.
const SCORING_SERVER_URL = "http://localhost:8000/";

async function fetchServerScores({ imageDataUrl, faceLandmarks, faceBlendshapes }) {
  if (!SCORING_SERVER_URL) return null;

  try {
    const response = await fetch(SCORING_SERVER_URL, {
      method: "POST",
    ...
    if (!response.ok) {
      console.warn("[ScoringServer] HTTP", response.status);
      return null;
    }

    const data = await response.json();
    if (!data?.results || !data?.scores) return null;
    return data;
  } catch (err) {
    console.warn("[ScoringServer] Request failed:", err);
    return null;
  }
}
...
async function handleClick(event) {
  ...
  const imageDataUrl = pixelCanvas.toDataURL("image/jpeg", 0.9);
  const serverResult = await fetchServerScores({
    imageDataUrl,
  ...
  renderGeneratedResult("image", result.faceLandmarks, result.faceBlendshapes, skinAnalysis, serverResult);
...
async function predictWebcam() {
  ...
  if (results?.faceBlendshapes?.length && now - lastVideoAnalysisMs > 250) {
    ...
    if (now - lastServerAnalysisMs > 800) {
      lastServerAnalysisMs = now;
      const imageDataUrl = vpcCanvas.toDataURL("image/jpeg", 0.85);
      serverResult = await fetchServerScores({
        imageDataUrl,
    ...
    renderGeneratedResult("video", results.faceLandmarks, results.faceBlendshapes, videoSkinAnalysis, serverResult);
  }
...
function renderGeneratedResult(mode, faceLandmarks, faceBlendshapes, skinAnalysis, externalResult) {
  const data = externalResult || generateFacialResults(faceLandmarks, faceBlendshapes, skinAnalysis);
