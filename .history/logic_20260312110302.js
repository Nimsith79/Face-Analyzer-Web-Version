...
function samplePatchPixels(cx, cy, radius, imgW, imgH, pixels) {
  ...
  // Blemish detection: count pixels that deviate strongly from the patch median
  ...
  for (let i = 0; i < n; i++) {
    ...
    // A pixel is a blemish candidate if it deviates > 28 color units from median
    // AND has a reddish/dark tint relative to local skin
    if (dist > 28) {
      blemishCount++;
    }
  }
  const blemishRatio = blemishCount / Math.max(n, 1);

  // Edge density: fraction of pixels with strong gradient (texture roughness)
  const edgeThreshold = 12;
  let edgeCount = 0;
  for (const g of gradients) {
    if (g > edgeThreshold) edgeCount++;
  }
  const edgeDensity = edgeCount / Math.max(gradients.length, 1);
...
export function analyzeSkinFromPixels(imageData, landmarks, imgWidth, imgHeight) {
  ...
  const radius = Math.max(8, Math.round(facePixelWidth * 0.07));

  // Sample 14 patches across the full face skin area
  const patchLandmarks = [
    // Left cheek (3 positions)
    116, 123, 50,
    // Right cheek (3 positions)
    345, 352, 280,
    // Forehead (3 positions)
    10, 67, 297,
    // Nose bridge
    6,
    // Chin area
  ...
  for (const idx of patchLandmarks) {
    if (!lm[idx]) continue;
    const patch = samplePatchPixels(
      Math.round(lm[idx].x * w), Math.round(lm[idx].y * h),
      radius, w, h, pixels
    );
    if (patch) skinPatches.push(patch);
  }

  if (skinPatches.length < 3) return null;
  ...
  const avgEdgeDensity = mean(skinPatches.map(p => p.edgeDensity));
  const avgBlemishRatio = mean(skinPatches.map(p => p.blemishRatio));
  const maxBlemishRatio = Math.max(...skinPatches.map(p => p.blemishRatio));

  // Uniformity: high variance = blotchy/uneven skin
  const uniformityScore = clamp(1 - avgVariance / 40, 0, 1);

  // Redness: elevated red channel = inflammation
  const rednessScore = clamp(1 - (avgRedness - 0.34) * 10, 0, 1);

  // Smoothness: based on edge density (% of rough pixels) rather than mean gradient
  const smoothnessScore = clamp(1 - avgEdgeDensity * 4, 0, 1);

  // Blemish score: directly penalizes detected spots/pimples
  // Uses both average across all patches and worst patch (catches localized acne)
  const blemishScore = clamp(1 - avgBlemishRatio * 5 - maxBlemishRatio * 2, 0, 1);
  ...
  const smallR = Math.max(5, radius - 3);
  ...
  return { uniformityScore, rednessScore, smoothnessScore, blemishScore, combinedScore, underEyeDarkness };
}
